...
function loginUser(credentials) {
    return new Promise((resolve, reject) => {
        console.log("db js login user method",credentials)
        if (credentials.email === '') {
            console.log("email is wrooooooong")
            let error = {
                error: true,
            ...
            db.collection('user').findOne({email: credentials.email}, ((err, doc) => {
                console.log("found something oh what to do")
                console.log(doc)
                console.log(err)
                if (err) {
                    console.log('err', err)
                ...
                if (credentials.facebookLogin) {
                    ...
                    if (doc) {
                        ...
                        let error = {
                            error: true,
                            message: 'No Facebook account connected please sign up.'
                        }
                        return reject(error)
                    }
                }
